...
import Firebase
func deleteFridge(_ controller: UIViewController, _ fridgeID: String, completion: @escaping () -> Void) {
    checkIsFridgeOwner(controller, fridgeID) {
...
func checkIsFridgeOwner(_ controller: UIViewController, _ fridgeID: String, _ completion: @escaping () -> Void) {
    ...
    userDoc.getDocument { (documentSnapshot, error) in
        ...
        guard let ownFridges = data["own_fridges"] as? [String] else { return }
        
        if ownFridges.contains(fridgeID) {
            completion()
        } else {
            alert("使用者非創建者，無法刪除冰箱", controller)
        }
